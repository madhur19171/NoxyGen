...
int main() {
	...
	for(int i=0;i<path_nodes_num;i++){
		...
		//You need to generate a generic verifier. In this implementation only two messages are sent from each node.
		//Instead, we want to read a file of stimuli and feed that file into the data in ports.
		//Read about File IO in Verilog and implement it that way.
		...
		//You have hard coded Destination to be Node 5. Destination has to be randomly selected.

		MyFile<<"\n\t\t\t\t\tNode"<<k<<"_data_in=3'd"<<5<<"\n\t\t\t\tend\n\t\t\t\telse";

		//16 Represents a unique constant for that set of messages. Do not hardcode it.
		//Create a list of current unique constant that have already been assigned to some message previously.
		//Then assign a new unique constant that has not been used previously
